-- Enforce Who Can Call Me and block relationships at the database trust boundary.

CREATE SCHEMA IF NOT EXISTS private;
REVOKE ALL ON SCHEMA private FROM PUBLIC;
GRANT USAGE ON SCHEMA private TO authenticated;

CREATE OR REPLACE FUNCTION private.call_allowed(
  p_caller uuid,
  p_callee uuid,
...
BEGIN
  IF p_caller IS NULL OR p_callee IS NULL OR p_caller = p_callee THEN
    RETURN false;
  END IF;
  ...
  SELECT
    coalesce(nullif(ufs.settings->'privacy'->>'whoCanCallMe', ''), 'Everyone'),
    coalesce(ufs.settings->'privacy'->'whoCanCallMeExceptions', '[]'::jsonb)
  INTO v_audience, v_exceptions
  FROM public.user_feature_settings ufs
  WHERE ufs.user_id = p_callee;
  ...
  SELECT EXISTS (
    ...
    WHERE cc.conversation_id = p_conversation_id
      ...
      AND (
        (cc.user_low_id = p_caller AND cc.user_high_id = p_callee)
        OR
        (cc.user_high_id = p_caller AND cc.user_low_id = p_callee)
      )
  ) INTO v_is_contact;

  IF v_audience IN ('My Contacts', 'My Contacts Except...') AND NOT v_is_contact THEN
    RETURN false;
  END IF;

  IF v_audience = 'My Contacts Except...'
     AND v_exceptions @> jsonb_build_array(p_caller::text) THEN
    RETURN false;
  END IF;
...
WITH CHECK (
  ...
  AND private.call_allowed(
    (SELECT auth.uid()),
    call_sessions.callee_id,
