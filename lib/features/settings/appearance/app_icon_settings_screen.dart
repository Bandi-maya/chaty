...
class _AppIconSettingsScreenState extends State<AppIconSettingsScreen> {
  ...
  Future<void> _applyPreset(ChatyAppIconPreset preset) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(error.toString().replaceFirst('PlatformException: ', ''))),
      );
    } finally {
  ...
  Future<void> _pickCustomIcon() async {
    ...
    if (bytes == null || bytes.isEmpty || !mounted) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Unable to read that image.')));
      return;
    }
    if (bytes.lengthInBytes > 20 * 1024 * 1024) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Choose an image smaller than 20 MB.')));
      return;
    }

    final croppedPath = await Navigator.of(context).push<String>(
      MaterialPageRoute(builder: (_) => _AppIconCropScreen(bytes: bytes!)),
    );
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Custom icon applied inside Chaty and saved permanently until you change it or uninstall the app.')),
      );
    } finally {
      if (mounted) setState(() => _busy = false);
    }
          ...
          builder: (context, _) => ListView(
            ...
            children: [
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              _service.selection == ChatyAppIconService.customSelection ? 'Your custom image' : _service.activePreset.name,
                  ...
                  return InkWell(
                    ...
                    child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        borderRadius: BorderRadius.circular(18),
                        border: Border.all(
                          color: selected ? theme.colorScheme.primary : theme.dividerColor.withValues(alpha: .35),
                          width: selected ? 2 : 1,
                        ),
                      ...
                      child: Column(
                        ...
                        children: [
                          Container(
                            ...
                            child: Icon(preset.icon, color: theme.colorScheme.onPrimary, size: 30),
                          ),
                  ...
                  children: [
                    ...
                    if (_service.hasCustomIcon)
                      ListTile(
                        ...
                        title: Text('Remove custom icon', style: TextStyle(color: theme.colorScheme.error, fontWeight: FontWeight.w700)),
                        onTap: _busy ? null : _removeCustom,
                      ),
                  ],
                ),
              ),
              ...
              Container(
                ...
                child: const Text(
                  'Android launcher icons must come from resources packaged inside the APK. The built-in Chaty icons can therefore replace the real launcher icon. A photo you upload can be used everywhere inside Chaty and persists across restarts, but Android does not allow an arbitrary runtime image to replace the installed launcher resource without rebuilding/reinstalling the APK.',
...
class _AppIconCropScreenState extends State<_AppIconCropScreen> {
  ...
  @override
  Widget build(BuildContext context) {
    final side = (MediaQuery.sizeOf(context).width - 36).clamp(220.0, 420.0);
    return Scaffold(
      appBar: AppBar(
        title: const Text('Crop app icon'),
