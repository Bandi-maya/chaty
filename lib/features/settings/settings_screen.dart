...
class SettingsScreen extends StatelessWidget {
  ...
  const SettingsScreen({super.key, required this.preferencesController, required this.themeController, required this.dataStore, required this.notificationService});

  void _open(BuildContext context, Widget page) => Navigator.of(context).push(MaterialPageRoute(builder: (_) => page));

  Widget _featureSection(String section) => GbSettingsHubScreen(preferencesController: preferencesController, initialSection: section);
  ...
  List<SettingsSearchResult> _buildSearchIndex(BuildContext context) => <SettingsSearchResult>[
        ...
        SettingsSearchResult(title: 'App icon', category: 'Appearance', description: 'Choose a Chaty launcher icon or upload a custom in-app icon', icon: Icons.apps_rounded, destination: const AppIconSettingsScreen()),
        ...
        SettingsSearchResult(title: 'All customization settings', category: 'Settings', description: 'Search every retained option by category and subcategory', icon: Icons.manage_search_rounded, destination: GbSettingsHubScreen(preferencesController: preferencesController)),
      ];
            ...
            child: Center(
              child: ConstrainedBox(
                ...
                child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                  ...
                  TextField(controller: nameCtrl, enabled: !saving, maxLength: 60, textInputAction: TextInputAction.next, style: const TextStyle(fontSize: 16), decoration: const InputDecoration(labelText: 'Display name', helperText: 'This is the name other Chaty users see.', border: OutlineInputBorder())),
                  const SizedBox(height: 12),
                  TextField(controller: aboutCtrl, enabled: !saving, maxLength: 160, minLines: 2, maxLines: 4, style: const TextStyle(fontSize: 16), decoration: const InputDecoration(labelText: 'About', border: OutlineInputBorder())),
                  if (errorText != null) ...[const SizedBox(height: 10), Text(errorText!, style: TextStyle(color: Theme.of(sheetContext).colorScheme.error, fontWeight: FontWeight.w600))],
                  const SizedBox(height: 18),
                  SizedBox(height: 52, child: FilledButton(
                    onPressed: saving ? null : () async {
                      final name = nameCtrl.text.trim();
                      if (name.length < 2) { setSheetState(() => errorText = 'Display name must contain at least 2 characters.'); return; }
                      ...
                      try {
                        ...
                        setSheetState(() { saving = false; errorText = error.toString().replaceFirst('Exception: ', ''); });
                      }
                    },
                    child: saving ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2)) : const Text('Save profile', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
                  )),
                ]),
              ),
            ),
  ...
  Future<void> _confirmLogout(BuildContext context) async {
    final confirmed = await showDialog<bool>(context: context, builder: (dialogContext) => AlertDialog(
      ...
      actions: [TextButton(onPressed: () => Navigator.pop(dialogContext, false), child: const Text('Cancel')), FilledButton(onPressed: () => Navigator.pop(dialogContext, true), child: const Text('Log out'))],
    ));
  ...
  Future<void> _copySettingsBackup(BuildContext context) async {
    final payload = <String, Object?>{'format': 'chaty-settings-v1', 'exportedAt': DateTime.now().toUtc().toIso8601String(), 'customization': preferencesController.gbFeatures};
    ...
    return Scaffold(
      ...
      body: SafeArea(child: Align(alignment: Alignment.topCenter, child: ConstrainedBox(
        ...
        child: ListView(keyboardDismissBehavior: ScrollViewKeyboardDismissBehavior.onDrag, padding: const EdgeInsets.fromLTRB(16, 12, 16, 104), children: [
          ...
          Card(elevation: 0, child: Padding(padding: const EdgeInsets.all(16), child: Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(user.displayName.isNotEmpty ? user.displayName : 'User', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800)), const SizedBox(height: 3), Text('@${user.username}', maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 14, color: theme.secondaryTextColor)), if (user.about.isNotEmpty) ...[const SizedBox(height: 4), Text(user.about, maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 14, color: theme.secondaryTextColor))]]) ),
          ]))),
          const SizedBox(height: 16),
          _sectionLabel(context, 'ACCOUNT'),
          ChatySettingsCard(children: [_tile(Icons.edit_outlined, 'Edit profile', 'Change your display name and About text', () => _showEditProfileDialog(context)), _tile(Icons.logout_rounded, 'Log out', 'Sign out of this account on this device', () => _confirmLogout(context), destructive: true)]),
          const SizedBox(height: 22),
          _sectionLabel(context, 'PRIVACY & SECURITY'),
          ChatySettingsCard(children: [_tile(Icons.shield_outlined, 'Privacy Center', 'Read receipts, last seen, stealth and core privacy', () => _open(context, PrivacyCenterScreen(preferencesController: preferencesController))), _tile(Icons.lock_outline_rounded, 'Security Center', 'App lock, protected access and security controls', () => _open(context, SecurityCenterScreen(preferencesController: preferencesController))), _tile(Icons.tune_rounded, 'More privacy options', 'Additional privacy controls grouped into subcategories', () => _open(context, _featureSection('Privacy & security'))), _tile(Icons.vpn_key_outlined, 'Permissions & hardware', 'Camera, microphone, notifications and system access', () => _open(context, SystemPermissionsScreen(preferencesController: preferencesController, notificationService: notificationService)))]),
          const SizedBox(height: 22),
          _sectionLabel(context, 'APPEARANCE & HOME'),
          ChatySettingsCard(children: [
            _tile(Icons.apps_rounded, 'App icon', 'Choose a Chaty launcher icon or upload, crop and persist your own image', () => _open(context, const AppIconSettingsScreen())),
          ...
          ChatySettingsCard(children: [_tile(Icons.chat_bubble_outline_rounded, 'Conversation settings', 'Message spacing, interactions, wallpaper and quick contacts', () => _open(context, ConversationSettingsPage(preferencesController: preferencesController))), _tile(Icons.tune_rounded, 'More chat options', 'Message behavior, headers, composer colors and advanced messaging', () => _open(context, _featureSection('Chats & messaging'))), _tile(Icons.smart_toy_outlined, 'Automation & scheduler', 'Auto replies, scheduled messages and workflow behavior', () => _open(context, MessageManagementPage(preferencesController: preferencesController, dataStore: dataStore))), _tile(Icons.bolt_outlined, 'More automation options', 'Translation and universal behavior controls', () => _open(context, _featureSection('Automation & behavior')))]),
          const SizedBox(height: 22),
          _sectionLabel(context, 'STATUS, CALLS & MEDIA'),
          ChatySettingsCard(children: [_tile(Icons.auto_stories_outlined, 'Status & stories', 'Status layout, rings, thumbnails and publishing behavior', () => _open(context, _featureSection('Status & stories'))), _tile(Icons.call_outlined, 'Calls', 'Call privacy, audio/video appearance and call controls', () => _open(context, _featureSection('Calls'))), _tile(Icons.perm_media_outlined, 'Media & storage', 'Upload quality, file limits, hidden media and cleanup', () => _open(context, _featureSection('Media & storage')))]),
          const SizedBox(height: 22),
          _sectionLabel(context, 'NOTIFICATIONS & NAVIGATION'),
          ChatySettingsCard(children: [_tile(Icons.notifications_none_rounded, 'Notification studio', 'Conversation sounds, heads-up alerts and notification behavior', () => _open(context, NotificationSettingsPage(preferencesController: preferencesController, notificationService: notificationService))), _tile(Icons.notifications_active_outlined, 'Presence & alerts', 'Online indicators, badges and presence notifications', () => _open(context, _featureSection('Notifications & presence'))), _tile(Icons.touch_app_outlined, 'Touch effects', 'Tap effects, particles and interaction feedback', () => _open(context, NavigationEffectsPage(preferencesController: preferencesController))), _tile(Icons.swipe_outlined, 'Navigation & gestures', 'Navigation styles, animations, gestures and movement behavior', () => _open(context, _featureSection('Navigation & gestures')))]),
          const SizedBox(height: 22),
          _sectionLabel(context, 'DATA & SYSTEM'),
          ChatySettingsCard(children: [_tile(Icons.manage_search_rounded, 'All customization settings', 'Search every retained option by category and subcategory', () => _open(context, GbSettingsHubScreen(preferencesController: preferencesController))), _tile(Icons.backup_outlined, 'Backup customization settings', 'Copy the current customization configuration as JSON', () => _copySettingsBackup(context)), _tile(Icons.restore_page_outlined, 'Reset & diagnostics', 'Reset privacy, customization, theme or all preferences', () => _showResetOptions(context)), _tile(Icons.info_outline_rounded, 'About Chaty', 'Version and application information', () => showAboutDialog(context: context, applicationName: 'Chaty', applicationVersion: '1.0.0 (Build 2026)', applicationLegalese: '© 2026 LOGY BYTE. All rights reserved.'))]),
        ]),
      ))),
    );
  }

  Widget _sectionLabel(BuildContext context, String title) => Padding(padding: const EdgeInsets.fromLTRB(8, 0, 8, 9), child: Text(title, style: Theme.of(context).textTheme.labelMedium?.copyWith(fontWeight: FontWeight.w800, letterSpacing: .8)));

  ChatySettingsTile _tile(IconData icon, String title, String subtitle, VoidCallback onTap, {bool destructive = false}) => ChatySettingsTile(icon: icon, iconColor: destructive ? Colors.redAccent : themeController.globalTheme.accentColor, title: title, subtitle: subtitle, onTap: onTap);
}
