...
class ContactRelationshipService {
  ...
  Future<ContactPrivacyOverride> privacyFor(String targetUserId) async {
    final row = await _client
        ...
        .maybeSingle();
    if (row == null) {
      return ContactPrivacyOverride(ownerUserId: _userId, targetUserId: targetUserId);
    }
    return ContactPrivacyOverride.fromDatabaseMap(Map<String, dynamic>.from(row));
  }

  Future<void> savePrivacy(ContactPrivacyOverride value) async {
    if (value.ownerUserId != _userId) throw StateError('Cannot change another user\'s privacy settings.');
    await _client.from('contact_privacy_overrides').upsert(
          value.toDatabaseMap(),
  ...
  Future<ContactConnectionStatus> connectionStatus(String otherUserId) async {
    ...
    if (raw is List && raw.isNotEmpty && raw.first is Map) {
      return ContactConnectionStatus.fromDatabaseMap(Map<String, dynamic>.from(raw.first as Map));
    }
    if (raw is Map) {
      return ContactConnectionStatus.fromDatabaseMap(Map<String, dynamic>.from(raw));
    }
  ...
  Future<String> acceptConnection(String otherUserId) async {
    ...
    if (id.isEmpty) throw StateError('Unable to create or accept the contact connection.');
    return id;
  }
    ...
    return (rows as List)
        ...
        .map((raw) {
          ...
          return LinkedDevice(
            ...
            lastActiveAt: DateTime.tryParse(row['last_active_at']?.toString() ?? '')?.toLocal() ?? DateTime.now(),
  ...
  Future<void> revokeDevice(String deviceId) async {
    final current = await currentDeviceId();
    if (deviceId == current) throw StateError('The current device cannot revoke itself from this screen.');
    await _client
        .from('linked_devices')
        .update(<String, dynamic>{'revoked_at': DateTime.now().toUtc().toIso8601String()})
        .eq('user_id', _userId)
