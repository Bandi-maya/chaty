...
import 'chaty_notification_service.dart';
...
class RichChatRealtimeService extends ChangeNotifier {
  ...
  final Map<String, PresenceState> _presenceByUserId = <String, PresenceState>{};
  final Map<String, DateTime?> _lastSeenByUserId = <String, DateTime?>{};
  final Map<String, ContactActivityState> _activityByConversationAndUser = <String, ContactActivityState>{};
  final Map<String, Map<String, dynamic>> _metadataByMessageId = <String, Map<String, dynamic>>{};
  final Map<String, DeliveryState> _deliveryStateByMessageId = <String, DeliveryState>{};
  ...
  PresenceState presenceFor(String userId) => _presenceByUserId[userId] ?? PresenceState.offline;
  bool isOnline(String userId) => presenceFor(userId) == PresenceState.online || presenceFor(userId) == PresenceState.typing;
  DateTime? lastSeenFor(String userId) => _lastSeenByUserId[userId];

  ContactActivityState activityFor(String conversationId, String userId) =>
      _activityByConversationAndUser['$conversationId:$userId'] ?? ContactActivityState(updatedAt: DateTime.fromMillisecondsSinceEpoch(0));

  Map<String, dynamic> metadataFor(String messageId) =>
      Map<String, dynamic>.unmodifiable(_metadataByMessageId[messageId] ?? const <String, dynamic>{});

  DeliveryState? deliveryStateFor(String messageId) => _deliveryStateByMessageId[messageId];
  ...
  Future<void> _initializeForSession() async {
    ...
    try {
      ...
      for (final conversationId in _trackedConversationIds.toList(growable: false)) {
        await _loadConversationRuntime(conversationId);
      }
      if (!_disposed) notifyListeners();
    } catch (error, stackTrace) {
      debugPrint('Chaty rich realtime initialization failed: $error\n$stackTrace');
    } finally {
      _initializing = false;
    }
  }
  ...
  Future<void> _loadPresenceProjection({required bool emitNotifications}) async {
  ...
  void _applyPresenceRow(Map<String, dynamic> row, {required bool emitNotification}) {
    ...
    if (emitNotification && previous == PresenceState.offline && next == PresenceState.online && _preferences.notification.notifyContactOnline) {
  ...
  Future<void> _loadActivity(String conversationId, {required bool emitNotifications}) async {
    final rows = await _client.from('typing_states').select().eq('conversation_id', conversationId);
    ...
    final stale = _activityByConversationAndUser.keys.where((key) => key.startsWith(prefix) && !activeKeys.contains(key)).toList();
    for (final key in stale) _activityByConversationAndUser.remove(key);
  }

  String? _applyActivityRow(Map<String, dynamic> row, {required bool emitNotification}) {
    final conversationId = row['conversation_id']?.toString() ?? '';
    final userId = row['user_id']?.toString() ?? '';
    if (conversationId.isEmpty || userId.isEmpty || userId == _currentUserId) return null;
    ...
    final profile = _backend.getUserById(userId);
    if (emitNotification && next.isTyping && previous?.isTyping != true && _preferences.notification.notifyTypingStarted) {
      _notifications.triggerEventNotification(
    ...
    if (emitNotification && next.isRecording && previous?.isRecording != true && _preferences.gbBool('notify_recording_started', fallback: true)) {
      _notifications.triggerEventNotification(
  ...
  Future<void> _loadMessageRuntime(String conversationId) async {
    ...
    final receipts = await _client.from('message_receipts').select('message_id,user_id,delivered_at,read_at').inFilter('message_id', messageIds);
  ...
  void _applyReceiptRow(Map<String, dynamic> row) {
    ...
    if (messageId.isEmpty || _senderByMessageId[messageId] != _currentUserId) return;
    if (row['read_at'] != null) {
      _deliveryStateByMessageId[messageId] = DeliveryState.read;
    } else if (row['delivered_at'] != null && _deliveryStateByMessageId[messageId] != DeliveryState.read) {
      _deliveryStateByMessageId[messageId] = DeliveryState.delivered;
    }
  }
  ...
  Future<void> markConversationDelivered(String conversationId) async {
    if (_currentUserId == null) return;
    try {
      await _client.rpc('mark_conversation_delivered', params: <String, dynamic>{'p_conversation_id': conversationId});
    } catch (error) {
  ...
  Future<void> setRecording(String conversationId, bool isRecording) async {
    if (_currentUserId == null) return;
    try {
      await _client.rpc('set_recording_state', params: <String, dynamic>{
        ...
        .onPostgresChanges(
          ...
          callback: (payload) {
            ...
            if (payload.eventType == PostgresChangeEvent.delete) {
              ...
              _applyPresenceRow(Map<String, dynamic>.from(row), emitNotification: true);
            }
            if (!_disposed) notifyListeners();
          },
        )
        .onPostgresChanges(
          ...
          callback: (payload) {
            ...
            if (payload.eventType == PostgresChangeEvent.delete) {
              ...
              _applyActivityRow(Map<String, dynamic>.from(row), emitNotification: true);
            }
            if (!_disposed) notifyListeners();
          },
        )
        ...
        .onPostgresChanges(
          ...
          callback: (payload) {
            ...
            if (payload.eventType == PostgresChangeEvent.delete) {
              ...
              _senderByMessageId[id] = row['sender_id']?.toString() ?? _senderByMessageId[id] ?? '';
              if (_senderByMessageId[id] == _currentUserId) _deliveryStateByMessageId.putIfAbsent(id, () => DeliveryState.sent);
            }
            if (!_disposed) notifyListeners();
          },
        )
